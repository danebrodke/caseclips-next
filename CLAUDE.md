...
- Chapters (desktop, `lg+`): sidebar beside the video, height matched via pure CSS (absolute-fill column inside the stretched flex row, whose height comes from the video's 16/9 aspect ratio — no JS measurement, so no post-hydration height jump in slow-hydrating browsers like Edge), auto-scrolls to active chapter
- Video page title block: stacked hierarchy — uppercase accent specialty kicker (same recipe as grid cards) above the serif title, then a byline row (36px avatar stack + author links, with institution · date as a smaller muted second line). No pill chips or single wrapping middot row
